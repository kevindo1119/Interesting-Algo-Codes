...
void modify(long long *polyR, long long *polyX, long long length) {
    for (long long i=0; i <length; ++i) {
        polyR[2*i] = polyR[2*i] - polyX[i];
    }
    for (long long i=0; i <2*length-1; ++i) {
        polyR[i] = polyR[i] / 2;
    }
    for (long long i=0; i <2*length-1; ++i) {
        polyR[i] = polyR[i] + polyR[i-1];
    }
    for (int i =0; i < length; ++i) {
        sub = sub + polyX[i] * polyR[i];
    }
}

long long sumX(long long *polyX,long long length) {
    long long temp=0;
    for (long long i=0; i < length; ++i) {
        temp = temp + polyX[i];
    }
...
int main(int argc, const char * argv[]) {
    ...
    initX(tempLength, polyX+1);
    total = sumX(polyX, tempLength+1);
    //   initY(tempLength + 1, polyY);
    initR(tempLength + 1, polyR);
    polyAll(polyR, polyX, polyX, tempLength + 1);
    //   printXYR(tempLength+1, polyX, polyY, polyR);
    //   printR(tempLength+1, polyR);
    modify(polyR, polyX, tempLength+1);
    
    
    cout << (total - 1) * (total - 2) * (total) / 6 - sub <<endl;
